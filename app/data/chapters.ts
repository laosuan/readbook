...
// Function to fetch Madame Bovary data translated by GPT-4.5 from CDN
async function fetchMadameBovaryGpt45Data() {
  try {
    // Direct access to CDN URL for GPT-4.5 translation
    const url = 'https://cdn.readwordly.com/MadameBovary_translate_cache_gpt45.json';
    
    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`Failed to fetch GPT-4.5 data: ${response.status} ${response.statusText}`);
    }
    
    const data = await response.json();
    return data;
  } catch (error) {
    console.error('Error fetching GPT-4.5 data:', error);
    return { paragraphs: [] }; // Return empty data structure on error
  }
}
...
// 将GPT-4.5翻译的JSON数据转换为BilingualContent对象
async function processGpt45JsonData(): Promise<BilingualContent[]> {
  const content: BilingualContent[] = [];
  
  // Fetch GPT-4.5 translated data from CDN
  const madameBovaryData = await fetchMadameBovaryGpt45Data();
  
  // 从新的JSON格式中获取段落数据
  if (!madameBovaryData.paragraphs) {
    console.error('Invalid GPT-4.5 data format: paragraphs array not found');
    return [];
  }
  ...
  madameBovaryData.paragraphs.forEach((paragraph: { id: string; source: string; translation: string }) => {
    ...
    if (paragraph.source && paragraph.translation) {
      content.push({
        id: `8-1-${paragraph.id}`, // 使用段落的id，注意这里的bookId是8
        english: paragraph.source, // 英文原文
        chinese: paragraph.translation, // 中文翻译
      });
    }
...
// 通过GPT-4.5翻译的JSON数据构建包法利夫人的全部章节内容
async function createMadameBovaryGpt45Chapters(): Promise<Chapter[]> {
  const allContent = await processGpt45JsonData();
  ...
  for (let i = 1; i <= partStructure.part1; i++) {
    ...
    const chapterContent = allContent.slice(startIndex, endIndex).map((item) => ({
      ...item,
      id: `8-1-${i}-${item.id.split('-').pop()}` // 使用原始段落ID作为标识的一部分
    }));
  ...
  if (markerIndices.partII > -1) {
    for (let i = 1; i <= partStructure.part2; i++) {
      ...
      const chapterContent = allContent.slice(startIndex, endIndex).map((item) => ({
        ...item,
        id: `8-2-${i}-${item.id.split('-').pop()}` // 使用原始段落ID作为标识的一部分
      }));
  ...
  if (markerIndices.partIII > -1) {
    for (let i = 1; i <= partStructure.part3; i++) {
      ...
      const chapterContent = allContent.slice(startIndex, endIndex).map((item) => ({
        ...item,
        id: `8-3-${i}-${item.id.split('-').pop()}` // 使用原始段落ID作为标识的一部分
      }));
  ...
  // Log the first few chapters to help with debugging
  if (chapters.length > 0) {
    console.log('First GPT-4.5 chapter:', {
      id: chapters[0].id,
      bookId: chapters[0].bookId,
      chapterNumber: chapters[0].chapterNumber,
      title: chapters[0].title,
    ...
    console.log('No GPT-4.5 chapters were created');
  }
...
export async function getChapters(): Promise<Chapter[]> {
  ...
  // Get both versions of Madame Bovary chapters
  const gpt4ominiChapters = await createMadameBovaryChapters();
  const gpt45Chapters = await createMadameBovaryGpt45Chapters();
  
  // Combine both sets of chapters
  cachedChapters = [...gpt4ominiChapters, ...gpt45Chapters];
